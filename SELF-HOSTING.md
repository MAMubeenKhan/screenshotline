...
| | |
| --- | --- |
| RAM | **2 GB minimum.** Each Chromium instance peaks near 1 GB before recycling; a 5-minute load test measured 1,344–1,619 MB with four browsers |
...
A 2 GB VPS costs $3–6/month, less than the hosted Starter plan. The server is
not the real cost, though: keeping Chrome updated, noticing failures and
debugging stuck pages is. Below 500 renders a month the hosted free tier costs
nothing at all — so self-host because you want control or air-gapping, not to
save money.
...
one-time `npm install` on the host. The smoke test starts its own local server;
the benchmark talks to the container over HTTP at `http://127.0.0.1:3000`:
